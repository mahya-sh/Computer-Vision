...
def display_region(org_image: np.ndarray,
    ...
    output_path = "Shadow_Region.jpg" 
    temp_filter = cv.cvtColor(temp_filter, cv.COLOR_BGR2RGB)
    cv.imwrite(output_path, temp_filter)
    print(f"Image saved at {output_path}")

    ax[2].imshow(cv.cvtColor(img_w_hole, cv.COLOR_BGR2RGB))
    ax[2].set_title("Shadow Region Cut")

    output_path = "Shadow_Region_Cut.jpg" 
    img_w_hole = cv.cvtColor(img_w_hole, cv.COLOR_BGR2RGB)
    cv.imwrite(output_path, img_w_hole)
    print(f"Image saved at {output_path}")

    ax[3].imshow(cv.cvtColor(shadow_clear_image, cv.COLOR_BGR2RGB))
    ax[3].set_title("Corrected Image")

    #output_path = "shadow_clear_image.jpg" 
    #shadow_clear_image = cv.cvtColor(shadow_clear_image, cv.COLOR_BGR2RGB)
    #cv.imwrite(output_path, shadow_clear_image)
    #print(f"Image saved at {output_path}")
...
image_path = "C:\\Users\\mahya\\Downloads\\test2.jpg"

org_image, mask, image_clear = process_image_file(image_path, ab_threshold = 0, save = True, verbose = True)